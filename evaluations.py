...
def Splitted_Sparse_Jacobian_evaluation_vectorized(X, observations,cl_ind_kind, K, cl_indices, cl_ptr, eq_compl, compl_indices, compl_ptr):
    ...
    obs_variables,obs_value,obs_kind,obs_variance = observations
    cl_data = [[] for k in range(K)]
    cl_J = [[] for k in range(K)]
    compl_J = [[] for k in range(K)]
    compl_data = [[] for k in range(K)]

    for k in range(K):
        # TD
        if cl_ind_kind[k][0]:
            v = np.array([X[obs_variables[i]] for i in cl_ind_kind[k][0]])
            J_loc = TD_jac_eval_vectorized(v, obs_variance[cl_ind_kind[k][0]])
            cl_data[k] = np.concatenate((cl_data[k], J_loc))
        # BE
        if cl_ind_kind[k][2]:
            v = np.array([X[obs_variables[i]] for i in cl_ind_kind[k][2]])
            J_loc = BE_jac_eval_vectorized(v, obs_variance[cl_ind_kind[k][2]])
            cl_data[k] = np.concatenate((cl_data[k], J_loc))
        # AN
        if cl_ind_kind[k][1]:
            v = np.array([X[obs_variables[i]] for i in cl_ind_kind[k][1]])
            J_loc = AN_jac_eval_vectorized(v, obs_variance[cl_ind_kind[k][1]], obs_value[cl_ind_kind[k][1]])
            cl_data[k] = np.concatenate((cl_data[k], J_loc))

        # PL
        if cl_ind_kind[k][3]:
            v = np.array([X[obs_variables[i]] for i in cl_ind_kind[k][3]])
            J_loc = PL_jac_eval_vectorized(v, obs_variance[cl_ind_kind[k][3]])
            cl_data[k] = np.concatenate((cl_data[k], J_loc))

        # EQ
        if cl_ind_kind[k][4]:
            res = [1, -1] * len(cl_ind_kind[k][4])
            res = res / obs_variance[cl_ind_kind[k][4][0]]
            cl_data[k] = np.concatenate((cl_data[k], np.ravel(res)  ))

        # PO
        if cl_ind_kind[k][5]:
            res = [1] * len(cl_ind_kind[k][5])
            cl_data[k] = np.concatenate((cl_data[k], res / obs_variance[cl_ind_kind[k][5]]))
        cl_J[k] = sp.csr_matrix((cl_data[k],cl_indices[k],cl_ptr[k]))
    ...
    for obs in eq_compl:
        ...
        if len(obs) == 3:
            ...
            if observations[-2][j] == 'TD':
                compl_data[k0].append(J_loc[aux] / obs_variance[j])
                aux += 1
                compl_data[k0].append(J_loc[aux] / obs_variance[j])
                aux += 1
                compl_data[k1].append(J_loc[aux] / obs_variance[j])
                aux += 1
                compl_data[k1].append(J_loc[aux] / obs_variance[j])
                aux += 1
                ...
                raise Exception('splitting ln433: len(obs)==3')
        else:
            j,k0,k1,k2 = obs
            aux = 0
            compl_data[k0].append(J_loc[aux]/obs_variance[j])
            ...
            compl_data[k1].append(J_loc[aux] / obs_variance[j])
            aux += 1
            compl_data[k2].append(J_loc[aux] / obs_variance[j])
            aux += 1
            compl_data[k2].append(J_loc[aux] / obs_variance[j])
            aux += 1
    for k in range(K):
        compl_J[k] = sp.csr_matrix((compl_data[k],compl_indices[k],compl_ptr[k]), shape=(np.shape(compl_ptr[k])[0]-1, np.shape(cl_J[k])[1]))
